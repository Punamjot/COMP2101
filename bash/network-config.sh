...
myLANAddress=$(ip a | grep inet | sed -n '3p' | awk '{print $2}')
myLANHostName=$(getent hosts | sed -n '2p' | awk '{print $2}')
myExternalIP=$(curl -s icanhazip.com) 
myExternalName=$(getent hosts $(curl -s icanhazip.com) | awk '{print $2}')
myRouterIP=$(ip route show | sed -n '1p' | awk '{print $3}')
myRouterName=$(nslookup $myRouterIP | sed -n '1p' | awk '{print $4}')
